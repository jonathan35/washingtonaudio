...
if($_POST['add2020']){
	$expUrl = explode('cms/', $_SERVER['REQUEST_URI']);
	$myUrl = $expUrl[1];
	header("Location:../".$myUrl);
}

$exist = sql_read("select * from content where row=? and pos=? limit 1", 'ss', array($_GET['row'], $_GET['pos']));

if(!empty($exist['id'])){
	$_GET['id'] = base64_encode($exist['id']);
}
...
$fields = array('id', 'status', 'content');//navigator
$value = array();
$type = array();
...
$fic_1 = array(0=>array(2, 0), 1 => array(1));//fic = fiels in column, number of fields by column $fic_1 normally for add or edit template
...
$_SESSION['module_content_id']='';
if(!empty($_GET['id'])){
	$_SESSION['module_content_id']=base64_decode($_GET['id']);
}

$attributes['position'] = array('placeholder' => 'A number for sorting');
//$placeholder['title'] = 'Title for profile content';
...
//$placeholder['post_content'] = 'Description for profile content';
...
$type['layout'] = 'select'; $option['layout'] = array('D1x2-M2x1'=>'Desktop 1 contents x 2 columns & Mobile 2 contents x 1 column', 'D1x3-M3x1'=>'Desktop 1 content x 3 columns & Mobile 3 contents x 1 column'); $default_value['layout'] = '1';
